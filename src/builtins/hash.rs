...
pub fn init(vm: &mut Vm) {
    ...
    vm.define_methods(c.hash, &[
        ...
        ("shift", |vm, s, _a, _b| { let first = with_mut(vm, s, |h| if h.entries.is_empty() { None } else { if !h.hashes.is_empty() { h.hashes.remove(0); } Some(h.entries.remove(0)) })?; match first { Some((k, v)) => Ok(vm.ary_new(vec![k, v])), None => Ok(Value::Nil) } }),
        ...
        ("dup", |vm, s, _a, _b| { let e = entries(vm, s); let d = default_of(vm, s); let c = vm.real_class_of(s); let ivars = s.obj().map(|o| vm.heap.get(o).ivars.clone()).unwrap_or_default(); let n = vm.heap.alloc(c, ObjKind::Hash(crate::object::HashData { entries: e, hashes: Vec::new(), default: d })); vm.heap.get_mut(n).ivars = ivars; Ok(Value::Obj(n)) }),
