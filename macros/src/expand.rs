...
fn ruby_methods_inner(attr: TokenStream, item: TokenStream) -> syn::Result<TokenStream> {
    ...
    let meta = if class_methods.is_empty() {
        ...
        quote! { let __meta = vm.singleton_class(::sabiruby::Value::Obj(__class)).expect("a class has a metaclass"); }
    };
    ...
    Ok(quote! {
        ...
        impl #self_ty {
            ...
            /// `impl` block. Generated by `#[ruby_methods]`; calling it twice is harmless.
...
fn define(m: &MethodSpec, self_ty: &Type, class_method: bool) -> TokenStream {
    ...
    if class_method {
        let call = if m.takes_vm {
            quote! { <#self_ty>::#rust_name(vm #(, #names)*) }
        } else {
            quote! { <#self_ty>::#rust_name(#(#names),*) }
        };
        return quote! {
            vm.define_fn(__meta, #ruby_name, |vm: &mut ::sabiruby::Vm #params| #ret {
                let __out = #call;
    ...
    let body = if m.takes_vm {
        ...
        quote! {
            #take
            let __out = <#self_ty>::#rust_name(#borrow, vm #(, #names)*);
    ...
    quote! {
        vm.define_fn(__class, #ruby_name, |vm: &mut ::sabiruby::Vm, #this #params| #ret {
            #body
            ::sabiruby::convert::IntoRubyRet::into_ruby_ret(__out, vm)
        });
    }
