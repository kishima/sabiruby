#!/bin/bash
# What classes, modules and methods SabiRuby has, next to the reference mruby 4.1.0-rc,
...
# tools/mrbtest.sh compiles with (kishima/mruby:4.1.0-rc), whose `mruby` is built from
...
arrows()  { if [ -s "$1" ]; then sort "$1" | awk -F'\t' "$CODE"'{ printf "* %s → %s\n", code($1), code($2) }'; else echo "(none)"; fi; }
{
  ...
  echo
  echo "Every class, module and method SabiRuby defines, next to the reference mruby 4.1.0-rc."
