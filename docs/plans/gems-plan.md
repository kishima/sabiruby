...
> [`../design/gems.md`](../design/gems.md)。移植後の作業は [`after-gems-plan.md`](after-gems-plan.md)。

対象: この文書だけを読んで、別セッションの実装者（AI）が SabiRuby に残りの本家 gem を移植できること。
作業前に `README.md`（Rules、Verification）、`docs/design/gems.md`（手順と、移植済み gem が教えたこと）、
`docs/design/gc.md`（ネイティブから見た GC の約束）を読むこと。設計判断はここに書いたとおりにし、
変えたい場合は理由を `docs/design/gems.md` に残す。

## 0. 前提と現状（着手時 2026-09-12、完了時の値を併記）

* 本家は mruby 4.1.0-rc（`../../ref/mruby`）。参照バイナリは Docker イメージ `kishima/mruby:4.1.0-rc`
  （バイト列ビルド、bigint 無し）。本家テストは `tools/mrbtest.sh` で走らせ、`docs/verification/mrbtest.md` に表を書く。
  ...
  **完了時は文字ビルド 2484 件中 2313 件／バイトビルド 2429 件中 2236 件**（`docs/verification/mrbtest.md`、`docs/verification/mrbtest-bytes.md`）。
  落ちる件の理由は `docs/verification/mrbtest-notes.md` と `tests/mrbtest/notes.tsv`。
* 移植済み（完了時）: `default.gembox` の 33 gem のうち POSIX 依存の 7 つ（io／socket／errno／dir／env／signal／process）を
  ...
  加えて UTF-8 文字列（ビルド構成、`docs/design/utf8.md`）と require／load（`docs/plans/eval-require-plan.md` 5 節）。
* 残り: 無い。POSIX 依存の 7 gem は対象外（著者決定）。
* 本家テストを 1 件でも落とす gem を「移植済み」と呼ばない。落ちる件は理由を `notes.tsv` に書き、
  意図した差異なら `docs/design/gems.md` の「Deviations kept」にも書く。
...
8. ネイティブ実行中は GC が走らない（`native_active`）。ネイティブが Ruby を呼び返す（`funcall`、`call_block`）間に
   保持する `ObjId` は、レジスタか `gc_register` で守る。長いループで大量に作るなら `docs/design/gc.md` の注意を読む。
9. 完了の定義: `tools/mrbtest.sh --update` で baseline 更新、`cargo test --release`、`SABIRUBY_GC_STRESS=1` で
   その gem のテストファイル、`tools/check_no_std.sh`、警告ゼロ。`docs/design/gems.md` に「What each gem needed」の項と
...
* 検証: `test/pack.rb` 全件。本家イメージとの照合スクリプトを `docs/design/gems.md` に残す
...
実装で決めたこと・分かったことは `docs/design/gems.md` の項と「Deviations kept」に書いた。以下は着手時の指示。
...
* 本家は `MRB_TT_RATIONAL`／`MRB_TT_COMPLEX` という専用の格納形（`struct RRational { mrb_int numerator, denominator }`、
  ...
  `count_objects` では `T_OBJECT` に数えられる差異を `docs/design/gems.md` に書く。
...
実装で決めたこと・分かったことは `docs/design/gems.md` の項と `docs/plans/eval-require-plan.md`。以下は着手時の指示。

* 設計は `docs/plans/eval-require-plan.md` に決めてある。要点:
...
* eval の VM 側は `src/builtins/ext_eval.rs`（設計 4.3）。フレームの環境（`REnv`）を Proc に付けて
  ...
  `env` の扱い（`docs/design/fibers.md`、`vm.rs` の `call_proc_inner`）を先に読む。
...
実装で決めたこと・分かったことは `docs/design/gems.md` の「mruby-bigint」の項と「Deviations kept」に書いた。
...
2. **`src/bigint.rs`**（VM 側、no_std）: 本家 `core/bigint.c` の順で写す。加減算（limb 単位の桁上がり）、乗算（本家は
   schoolbook + 大きい時 Karatsuba。schoolbook だけで始め、`docs/verification/bench.md` に測って足りなければ Karatsuba）、
   ...
   `gcd`／`lcm`。関数名は `mrb_bint_*` に対応させて `bint_add` などとし、`docs/design/gems.md` に対応表を書く。
3. **core の分岐**（`src/builtins/numeric.rs`、`vm.rs` の算術命令）: 本家 `numeric.c` の `#ifdef MRB_USE_BIGINT` 62 か所に相当。
   ...
   * `to_s(base)`、`inspect`、`to_f`、`to_i`（Float → Integer で `1e30.to_i` は多倍長。`expect_int` の RangeError 経路を確認）、
     `Integer#size`（本家は limb 数×4。`docs/design/gems.md` に差異があれば記録）、`digits`、`bit_length`、`pow(b, m)`、
...
5. **照合**: 参照イメージと次を比べるスクリプトを `docs/design/gems.md` に残す。`2**63`、`2**64 * 2**64`、`-(2**64) / 7` と
...
（`docs/design/gems.md` と本の corelib 章のメモに記録する）。
...
`tests/custom/utf8_reference_bugs.rb` と `docs/design/utf8.md` に記録した。詳細は `docs/design/utf8.md`。

### 3.6 mruby-regexp — エンジンは `regex-automata`、表面は本家を移植（著者決定 2026-09-13）— **済み（2026-09-13）**

結果は `docs/design/gems.md`（「What each gem needed」の mruby-regexp と「Deviations kept」）、
テストの内訳は `docs/verification/mrbtest-notes.md`、wasm のサイズは `docs/design/playground.md`。
...
* `regex-automata = { version = "0.4", default-features = false, features = ["alloc", "syntax", "meta", "nfa-pikevm", "nfa-backtrack"] }`。
  ...
  wasm のサイズは Playground で両方測って `docs/design/playground.md` に書く（未計測）。
...
* **`$~` と `$1`〜`$9`、`$&`、`` $` ``、`$'`**: 本家はメソッドフレームごとの特別変数（`OP_GETSV`／`OP_SETSV` → `mrb_vm_special_get/set`、
  `src/variable.c`）。SabiRuby ではこの 2 命令がまだ一度も実行されていない（`docs/verification/mrbtest.md` の never 一覧）。
  ...
  GC のルートに足す（`docs/design/gc.md` の表を更新）。
...
6. `docs/design/gems.md` に「Deviations kept」として構文の表と件数を書く。本の移植章には「正規表現エンジンは置き換えてよい実装都合だが、
...
`proc_set_stack` は 0 件（`respond_to?` で自分を飛ばす）。詳細と差異は `docs/design/gems.md`。
...
* `docs/design/gc.md` の「スケジューラ駆動の GC」（idle で回収）はここで入れる。
* 本家 `mrbgems/mruby-task/test/gc_task.rb` は GC の細部（`GC_STEP_SIZE`）に依存する。落ちる件は理由を書く。

## 4. 落とし穴（今日の 17 gem で踏んだもの。`docs/design/gems.md` に詳細）
...
* 実装で分かったことは `docs/design/gems.md`（gem ごとの項）、本の repo の `docs/notes/sabiruby-findings.md`（節 18 以降）、
  `data/porting_stages.yml` 段階 7 と `porting.re` の「踏みやすい点」へ。本文（`.re`）に Rust の名前は書かない。
* この文書は着手時に「実装済み」の注記を頭に足し、変えた判断は `docs/design/gems.md` に理由付きで書く（`gc-plan.md` と同じ運用）。
