...
* `sabiruby`（lib）は no_std + alloc で、`wasm32-unknown-unknown` のビルドは CI で通っている。GC 済み（`docs/design/gc.md`）。
* Ruby ソースからのコンパイルは `sabiruby-compiler`（`docs/design/compiler.md`）で、中身は本家の C（Prism 1.9.0 + codegen）。**ブラウザで Ruby を書いて動かすには、この C も wasm にする必要がある**。
  `docs/design/compiler.md` は wasm32 を対象外としているが、理由は「clang と wasm 用 sysroot が要る」であって、方式の問題ではない。
...
* `Vm::step` は Fiber の中で止まっても続きから走る（`docs/design/fibers.md`）ので、予算刻みは VM の意味論に影響しない。
...
* C（`sabiruby-compiler` の build.rs、`cc` crate）: 環境変数で wasi-sdk の clang を指定する。
  ...
  `sabiruby-compiler` の build.rs に手を入れる必要が無いことを最初に確かめる（`cc` は `CC_<target>` を読む）。必要なら `docs/design/compiler.md` の「wasm32 は対象外」を「wasi-sdk があれば可」に改める。
...
6. 文書: `docs/design/playground.md`（設計、API、ビルド手順、対応ブラウザ）。本のリポジトリ `docs/notes/sabiruby-findings.md` に §15（wasm 化で分かったこと。特に「本家コンパイラは wasi-sdk で wasm にできる」「スタックサイズ」「WASI の import で実際に要ったもの」）。
...
* `mrc_presym.c` の静的変数（`docs/design/compiler.md`）: 単一スレッドなので問題ないが、`sabi_compile` を再入しない。
...
5. `docs/design/playground.md` と本のノート §15 がある。
...
* `sabiruby` 本体と `sabiruby-compiler` の変更は最小にする（wasm 向けの build.rs の調整程度）。変更したら理由を `docs/design/compiler.md` に残す。
