...
| 2 | 実行ループの無駄取り | 着手（2026-09-15）。候補 0（段階 0 を取り戻す）と 3（`find_method`）を先に |
| 2b | Hash と String | 未着手（段階 2 の後、同じ担当） |
| 3 | ネイティブのクロージャと型付きホスト状態 | **済み**（2026-09-15、`93824b1` `1472346`）。sabiruby 側のみ。rubevy 側の置き換えは未 |
| 4 | `FromRuby` / `IntoRuby` と `define_fn` | 着手（2026-09-15） |
| 5 | Data オブジェクト（ハンドル方式）と解放フック | 着手（2026-09-15、段階 4 の後、同じ担当） |
