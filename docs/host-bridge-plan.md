...
| 2c | `Array#shift` の O(1) 化（開始オフセット）、Hash のハッシュ表化（入口の 1 本化 → 索引） | 着手（2026-09-15、著者判断: 案 1 と案 2、A → B の順） |
...
| 5 | Data オブジェクト（ハンドル方式）と解放フック | **済み**（2026-09-15、`c667a9d`）。rubevy 側（static の置き換え、エンティティの Data 化）は着手（2026-09-15、rubevy のブランチ `host-state`） |
